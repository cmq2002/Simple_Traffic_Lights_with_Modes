...
static void set_current_seven_segment_led(uint8_t index){
	if(index >= NUMBER_OF_SEVEN_SEGMENT_LEDS) return;
...
void seven_segment_led_driver(void){
	...
	ledIndex = (ledIndex + 1)%NUMBER_OF_SEVEN_SEGMENT_LEDS; //0 1 2 ..->7->0 1 2 .. ->7
//	ledIndex++;
//	if(ledIndex == NUMBER_OF_SEVEN_SEGMENT_LEDS)
//		ledIndex = 0;
